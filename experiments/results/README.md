...
| `search_benchmark_<task>.csv` | output of `experiments/search_benchmark.py` (2026-09-06): V7 against char-n-gram BM25 (with a tuned idf exponent -- textbook idf¹ under-tunes it by 0.05 MAP on records), tf-idf cosine, token BM25, brute-force Damerau-Levenshtein, and rerank / RRF / tuned-interpolation (`mix`) combinations, on 17 tasks. Document tasks (mean > 600 chars) use the `n ∈ {(3,),(4,),(3,4)}` tier. `--backfill` adds combination rows a CSV lacks; `--baselines` keeps a CSV's V7 rows and re-runs everything else. Every system's whole tuning grid on the `tune` half of the queries, and the selected configuration (`selected=True`) on the `test` half. Columns: `system`, `family`, `config` (JSON), `split`, `n_queries`, `map`, `mrr`, `ndcg@10`, `hit@1`, `recall@10`, `recall@50`, `ms_per_query`. Analysis in `docs/bow-plan.md` Part 10. |
| `search_benchmark_summary.md` | the test-split table across tasks and systems, regenerated by `search_benchmark.py --report` |
