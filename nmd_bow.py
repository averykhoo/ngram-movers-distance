...
def bow_ngram_movers_distance(bag_of_words_1: Union[str, Iterable[str]],
    ...
    # convert to list
    bag_of_words_1 = list(bag_of_words_1)  # columns
    bag_of_words_2 = list(bag_of_words_2)  # rows

    # optimize cost matrix using EMD
    costs = [[ngram_movers_distance(word_1, word_2, normalize=True)
...
if __name__ == '__main__':
    ...
    for ref_line, hyp_line in zip(ref_lines, hyp_lines):
        ...
        scores_bow.append(bow_ngram_movers_distance(ref_tokens, hyp_tokens) / max(len(ref_tokens), len(hyp_tokens)))
        scores_nmd.append(ngram_movers_distance(' '.join(ref_tokens), ' '.join(hyp_tokens), normalize=True))
        print(' '.join(ref_tokens))
        print(' '.join(hyp_tokens))
        print(bow_ngram_movers_distance(ref_tokens, hyp_tokens) / max(len(ref_tokens), len(hyp_tokens)))
        print(ngram_movers_distance(' '.join(ref_tokens), ' '.join(hyp_tokens), normalize=True))
    ...
    print(min(zip(scores_diff,scores_bow,scores_nmd,ref_lines,hyp_lines)))
    print(max(zip(scores_diff,scores_bow,scores_nmd,ref_lines,hyp_lines)))
